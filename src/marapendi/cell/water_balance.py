...
class MembraneWaterBalanceModel:
    ...
    def _solve_static_water_balance(self, cell, state):
        """Solve static water balance.

        Liquid saturation is computed after this call (by
        :meth:`calculate_water_transport`) for gas-transport resistance, but it
        does not feed back into the membrane water-content boundary condition.
        """
        ...
        # Both "phase" profile slots are set to the same solved profile so that
        # pem.proton_resistance (which weights them by saturation) works correctly.
