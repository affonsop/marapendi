...
class TransientCellModel:
    ...
    * ``i_s    = 6`` : liquid water saturation s [–]
    ...
    def _compute_derived_quantities(self, x, current_density) -> CellState:
    ...
    def _compute_fluxes(self, eff_R, state: CellState):
        ...
        (V_cell, eta_ohm, eta_act,
         ...
         eta_memb, _, eta_gdl) = cell.voltage_model.calculate_cell_voltage(
            ...
            current_density=state.iF * ct.faraday,
        ...
        S_T_losses[memb_ix,   ...] = eta_memb  * current_density
        S_T_losses[ca_cl_ix,  ...] = (eta_act + E_rev_ca) * current_density
        S_T_losses[an_cl_ix,  ...] = E_rev_an  * current_density
        S_T_losses[ca_gdl_ix, ...] = eta_gdl   * current_density
        S_T_losses[an_gdl_ix, ...] = eta_gdl   * current_density
    ...
    def rates_of_change(self, x, current_density=0.):
        ...
        state = self._compute_derived_quantities(x, current_density)
