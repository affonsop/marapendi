"""
State containers for the ``future`` cell model.

Mirrors the *shape* of :class:`marapendi.future.cell.Cell` (``ca``/``an``
sides, each with a catalyst layer, GDL/MPL and flow channel, plus a
membrane) so that state objects line up 1:1 with the components that own
...
This module currently only defines the dataclasses and their fields.
Correlation methods will be added component by component.
"""
...
class CellSideState:
    ...
    def layers(self) -> list[LayerState]:
        """All layer states on this side, GDL-to-CL order, including MPL and channel if present."""
