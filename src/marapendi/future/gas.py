...
surrounding :class:`~marapendi.future.state.LayerState` /
:class:`~marapendi.future.state.FlowChannelState`.
...
and ``pressure``. This avoids the duplication between
:class:`marapendi.legacy.gas_composition.GasComposition` and the
``PorousLayer`` getters, and ensures the saturation pressure is computed
from a single place.
"""
...
class GasModel:
    ...
    :class:`~marapendi.future.state.LayerState` or
    :class:`~marapendi.future.state.FlowChannelState`.
    """
    ...
    def set_composition(state, dry_o2_mole_fraction: float, dry_h2_mole_fraction: float,
        ...
        h2o_mole_fraction = relative_humidity * GasModel.saturation_pressure(state) / state.pressure
    ...
    def species_diffusion_coefficient(state, species: str) -> float:
        ...
        """
        if species == 'o2':
            reference_diffusion_coefficient = 0.28e-4  # [m^2/s]
        elif species == 'h2':
