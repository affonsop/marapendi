...
catalyst-layer/membrane interfaces, given a :class:`~marapendi.future.cell.Cell`
and its :class:`~marapendi.future.state.CellState`.

This is a direct port of :class:`marapendi.legacy.water_balance_models.MembraneWaterBalanceModel`,
adapted to read all physical variables from ``state`` (or compute them from
``cell.membrane`` correlations) instead of mutating a ``FuelCell``-shaped
object. Only the steady-state (``dynamic=False``) solution path is ported.
...
class MembraneWaterBalanceModel:
    ...
    def calculate_water_saturation(self, cell: Cell, state: CellState) -> None:
        ...
        Direct port of :meth:`marapendi.legacy.fuelcell.FuelCellSide.calculate_water_saturation`
        (cathode-only, as in the legacy ``dynamic=False`` water transport step).
