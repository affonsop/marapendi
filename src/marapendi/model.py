...
:class:`CellModel` operates on a :class:`~marapendi.cell.Cell` and a
:class:`~marapendi.state.CellState` to compute the cell's behaviour.
:class:`ExplicitSteadyStateModel` is the concrete steady-state
implementation: it composes the gas-transport, water-balance, voltage and
thermal sub-models into a single explicit evaluation.
"""
    ...
    def initial_state(self, conditions: CellOperatingConditions) -> CellState:
        ...
        for side_name, side_state in zip(('ca', 'an'), state.sides):
            ...
            for layer in side_state.layers:
                ...
                layer.pressure = side_conditions.cell_pressure
                ...
                GasModel.set_composition(
                    ...
                    side_conditions.inlet_pressure,
                    side_conditions.inlet_temperature,
                )

            if side_conditions.inlet_dry_molar_flow_rate is not None:
                side_state.ch.inlet_gas_flow_rate = (
                    side_conditions.inlet_dry_molar_flow_rate 
                    / side_conditions.inlet_gas_concentration
                    / (1-side_conditions.inlet_vapor_mole_fraction)
                )
            else:
                side_state.ch.inlet_gas_flow_rate = (
                    side_conditions.stoichiometry * side_reactant_consumption[side_name] * self.cell.area
                    / side_conditions.reactant_mole_fraction
                    / side_conditions.inlet_gas_concentration
                )
        return state
