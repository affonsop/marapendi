...
class TestMembraneWaterProperties:
    def test_water_vol_fraction_increases_with_content(self, pfsa_thick):
        fv5 = pfsa_thick.ionomer.water_vol_fraction(5, water_molar_volume(353.15))
        fv15 = pfsa_thick.ionomer.water_vol_fraction(15, water_molar_volume(353.15))
        assert fv15 > fv5

    def test_water_vol_fraction_known_value(self, pfsa_thin):
        # At λ=10, roughly 0.25 vol fraction for typical Nafion
        fv = pfsa_thin.ionomer.water_vol_fraction(10, water_molar_volume(353.15))
        assert 0.1 < fv < 0.5
...
class TestMembraneProtonConductivity:
    ...
    def test_liquid_equilibrium_water_content_increases_with_temperature(self, pfsa_thick):
        wc_low = pfsa_thick.ionomer.liquid_equilibrium_water_content(300.)
        wc_high = pfsa_thick.ionomer.liquid_equilibrium_water_content(353.15)
        assert wc_high > wc_low
...
class TestHydrogenPermeation:
    ...
    def test_flux_increases_with_partial_pressure(self, pfsa_thick):
        flux_low = pfsa_thick.hydrogen_permeation_flux(5e4, 353.15, 10)
        flux_high = pfsa_thick.hydrogen_permeation_flux(1e5, 353.15, 10)
        assert flux_high > flux_low

    def test_thin_membrane_higher_flux_than_thick(self, pfsa_thin, pfsa_thick):
        args = dict(h2_pressure_difference=1e5, temperature=353.15, water_content=10)
