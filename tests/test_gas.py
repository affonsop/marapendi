...
def test_gas_composition(gas): 
    ...
    assert np.isclose(gas.gas.X[1], 0.79 * (1-0.5 * cb.water_saturation_pressure(gas.temperature())/1e5), 1e-4)
    assert gas.gas.X[2] == 0
    assert np.isclose(gas.gas.X[-1], 0.5 * cb.water_saturation_pressure(gas.temperature())/1e5, 1e-4)
    assert np.isclose(gas.pressure(), 1e5)
